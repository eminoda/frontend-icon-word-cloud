...
function dominantColor(body: string): string | null {
...
for (const l of frontendLogos) {
  ...
  if (!theme && !dom) continue
  if (theme && !dom) {
    cannotVerify.push(l.name)
    continue
  }
  if (!theme && dom) {
    // Fill missing theme color with icon dominant
    overrides[l.name] = dom
    continue
  }

  if (theme !== dom && dist(theme!, dom!) > 40) {
    overrides[l.name] = dom!
  }
}

const sorted = Object.fromEntries(Object.entries(overrides).sort((a, b) => a[0].localeCompare(b[0])))

console.log(
  JSON.stringify(
    {
      total: frontendLogos.length,
      overrideCount: Object.keys(sorted).length,
      overrides: sorted,
      cannotVerifyCount: cannotVerify.length,
    },
    null,
    2,
  ),
